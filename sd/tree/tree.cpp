...
// ----------------- Вывод -----------------
void AVL::print_inorder() const {
  inorder(root, [](int v) { cout << v << " "; });
  cout << endl;
}
void AVL::print_preorder() const {
  preorder(root, [](int v) { cout << v << " "; });
  cout << endl;
}
void AVL::print_postorder() const {
  postorder(root, [](int v) { cout << v << " "; });
  cout << endl;
}
void AVL::print_lvlorder() const {
  lvlorder(root, [](int v) { cout << v << " "; });
  cout << endl;
}

// --------- Бинарная сериализация ---------
void AVL::serialize(std::ostream &out) const {
  std::function<void(AVLNode *, std::ostream &)> serialize_node =
      [&](AVLNode *n, std::ostream &os) {
        if (!n) {
          int marker = -999999;
